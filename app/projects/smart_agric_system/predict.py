...
model_img = load_model("pest_model_best.keras")
...
def final_decision(pest_result, irrigation_result):
    ...
    # 💧 Irrigation + disease interaction
...
#     proba = model_img.predict(img_array)[0]
#     # Get best prediction
#     class_id = int(np.argmax(proba))
#     confidence = float(proba[class_id])
#     pest_result= pest_decision(class_id,confidence)
#     irrigation_result= irrigation_decision(prob,crop)
#     return final_decision(pest_result=pest_result,irrigation_result=irrigation_result)
